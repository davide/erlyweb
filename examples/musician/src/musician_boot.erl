...
compile(Opts) ->
    % We are passing the AppName as the first argument
...
compile_dir(appconfig) ->
    {ok, CDir} = .application:get_env(musician, compile_dir),
    CDir;
compile_dir(Dir) ->
    Dir.


%% This function is needed in the mysql start_phase so I copied it here.
%% Its original location was twoorl.util.
